...
@section('content')
    ...
    <section class="min-h-[90vh] flex items-center justify-center text-center text-white relative overflow-hidden"
        ...
        <div class="max-w-3xl mx-auto px-6 py-20 relative z-10">
            @if($page->product)
                <span class="inline-block text-white px-4 py-1 rounded-full text-sm font-medium mb-6 backdrop-blur-sm" style="background: rgba(0,0,0,0.5);">
            ...
            @if($page->hero_subtitle)
                <p class="text-lg md:text-xl text-white mb-8 max-w-2xl mx-auto leading-relaxed">{{ $page->hero_subtitle }}</p>
            @endif
            ...
            <p class="mt-6"><a href="{{ route('products') }}" class="text-white hover:text-white underline text-sm">← Back to all products</a></p>
        </div>
    </section>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    @foreach($page->gallery as $img)
                        <div class="aspect-square rounded-xl overflow-hidden shadow-md">
                            <img src="{{ $img }}" alt="{{ $page->title }}" loading="lazy" width="300" height="300" class="w-full h-full object-cover hover:scale-105 transition-transform duration-300">
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    @if($page->product)
        <section class="py-16" style="background-color: #452aa8;">
            <div class="max-w-4xl mx-auto px-6 text-center">
                ...
                <p class="text-4xl font-bold my-6" style="color: #5ddf8e;">${{ number_format($page->product->price, 2) }}</p>
